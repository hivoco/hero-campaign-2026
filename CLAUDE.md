...
| `/privacy-policy-and-terms-conditions` | Legal | **Combined** Terms & Conditions **and** Privacy Notice (single DPDP-compliant doc, sections A–Q). Deliberately **light/white readable** page (the one non-dark screen besides the OTP sheet), full-width (not phone-framed): Bebas title, hero-red section markers, **one continuous flowing document — no table of contents, no in-page anchors** (per the "single route, no id-based navigation" decision), numbered clauses w/ bold lead-ins + nested bullets, emails auto-linked. Server component. Content lives verbatim in `src/lib/legal.ts` (**legally load-bearing — don't paraphrase**; source is `20260721_Terms and conditions.docx`). The details-form consent line has a **single** link labelled "Privacy Policy and Terms & Conditions" pointing to this route (opens in a new tab) — the **route slug matches that UI label order**. This is the **only** legal route — there is no `/terms` route or redirect. |
...
- **MRN Digital pixel**: fired **only on the Send OTP action** (not site-wide) via `fireMrnPixel()`
  (`src/lib/mrn-pixel.ts`, a `new Image()` request to `https://trk.mrndigital.in/pixel?av=…` with a
  cache-buster), called in `details-form.tsx` right beside the Meta `SEND_OTP_DestiniTech` event once the
  form is valid and submitting.
...
**`/privacy-policy-and-terms-conditions`** (combined T&C + Privacy Notice, light page) is linked from the
`/details` consent line (a single "Privacy Policy and Terms & Conditions" link goes to this one route),
opened in a new tab. Two things on
