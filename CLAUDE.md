...
- **Selfie capture + validation** (the "camera flow"): `capture-overlay.tsx` runs c0→c1→c2→c3 as a
  ...
  **`@vladmandic/face-api`** (chosen over MediaPipe/original face-api.js for a light client — the user's
  ...
  is a **stub** (~650ms) until a real endpoint exists — swap only that function body. QA: `?selfie=pass|fail|api-fail`.
...
`/language` → `/details` (Send OTP opens the OTP sheet **in place**) → `/thank-you`. Two things on
