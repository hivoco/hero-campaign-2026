 ...
 * chosen-world scene. There is no separate CTA: the choice *is* the action, so
 ...
 */
export function LanguageSelect({ world }: { world: World }) {
  return (
    <div className="relative mx-auto flex h-dvh w-full max-w-[440px] flex-col overflow-hidden bg-ink shadow-[0_0_60px_rgba(0,0,0,0.6)]">
      {/* Full-bleed background — the chosen world scene, blurred + darkened for legibility. */}
      ...
      <div className="relative z-10 flex flex-1 flex-col px-5 pt-9 pb-8">
        ...
        <h1 className="display mt-7 text-center text-[2.55rem] leading-[0.92] text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)] animate-rise [animation-delay:100ms]">
        ...
        <ul className="mt-6 flex flex-1 flex-col justify-center gap-3">
          {languages.map((lang, i) => (
            ...
            >
              <Link
                href={`/details?world=${world.slug}&lang=${lang.code}`}
                className="group relative flex min-h-[3.5rem] w-full items-center overflow-hidden rounded-panel border border-white/25 bg-white/[0.08] px-6 text-white shadow-[inset_0_1px_0_rgb(255_255_255/0.22)] backdrop-blur-[14px] backdrop-saturate-150 transition duration-200 ease-out-soft hover:border-hero-red/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-hero-red-bright focus-visible:ring-offset-2 focus-visible:ring-offset-black/40 active:scale-[0.99]"
              >
                ...
                />
                <span className="relative z-10 text-[1.1rem] tracking-wide text-white transition-colors duration-200 group-hover:text-white group-focus-visible:text-white">
                  {lang.label}
                </span>
              </Link>
            </li>
